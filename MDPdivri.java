...
public class MDP {
   ...
   break_label: for (Distribution distribution : steps.get(state)) 
	...
	public HashSet<State> Smax0(HashSet<State> T)
	{
		HashSet<State> R = new HashSet<State>();
		HashSet<State> R1 = new HashSet<State>();
		...
		do
		{
			...
			for (State state : states)
			{
				...
				for (Distribution distribution:steps.get(state))
				{
					ok = 0;
					for (StateProbability stateProbability:distribution.getStateProbabilities())
						if (R1.contains(stateProbability.getState()) && stateProbability.getProbability()>0)
						{	
							ok = 1;
							break;
						}
		...
		} while (!SetOperations.equal(R, R1));
	...
	public static void main(String[] args)
	{
		...
		*/
		...
		HashSet<State> T1 = new HashSet<State>();
		T1.add(s3);
		
		// Smax0 result
		HashSet<State> smax0Result = mdp.Smax0(T1);
		System.out.println("Smax0: " + smax0Result);
	}
}
